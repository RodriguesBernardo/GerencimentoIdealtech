...
<form action="{{ route('orcamentos.update', $orcamento->id) }}" method="POST" id="form-orcamento" class="fade-in">
    @csrf
    @method('PUT')
            ...
            <div class="card mb-4">
                ...
                <div class="card-body">
                    <div class="form-check form-switch mb-4">
                        <input class="form-check-input" type="checkbox" id="toggleClienteAvulso" {{ old('cliente_nome_avulso', $orcamento->cliente_nome_avulso) ? 'checked' : '' }}>
                    ...
                    <div id="div-cliente-avulso" class="row d-none">
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="text" name="cliente_nome_avulso" id="cliente_nome_avulso" class="form-control" value="{{ old('cliente_nome_avulso', $orcamento->cliente_nome_avulso) }}">
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="card mb-4">
                ...
                <div class="card-body">
                    <div class="mb-4">
                        ...
                        <div id="editor-observacoes" style="height: 150px;">{!! old('observacoes', $orcamento->observacoes) !!}</div>
                    ...
                    <div class="mb-3">
                        ...
                        <div id="editor-notas" style="height: 100px;">{!! old('notas_internas', $orcamento->notas_internas) !!}</div>
            ...
            <div class="card mb-4">
                ...
                <div class="card-body">
                    ...
                    <div class="form-check form-switch mb-3 p-3 bg-light border rounded">
                        <input class="form-check-input ms-0 me-2" type="checkbox" name="mostrar_valores_individuais" id="mostrar_valores" value="1" {{ old('mostrar_valores_individuais', $orcamento->mostrar_valores_individuais) ? 'checked' : '' }}>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="date" name="data_emissao" class="form-control" value="{{ old('data_emissao', $orcamento->data_emissao->format('Y-m-d')) }}" required>
                    </div>
                    <div class="mb-3">
                        <label class="form-label">Validade do Orçamento</label>
                        <input type="date" name="data_validade" class="form-control" value="{{ old('data_validade', $orcamento->data_validade ? $orcamento->data_validade->format('Y-m-d') : '') }}">
                    </div>
                    <div class="mb-3">
                        ...
                        <select name="status" class="form-select">
                            ...
                            <option value="Vencido" {{ old('status', $orcamento->status) == 'Vencido' ? 'selected' : '' }}>Vencido</option>
                        </select>
                    </div>

                    <div class="mb-3 border rounded p-2">
                        <label class="form-label fw-bold">Condições de Pagamento</label>
                        <div id="editor-pagamento" style="height: 100px;">{!! old('condicoes_pagamento', $orcamento->condicoes_pagamento) !!}</div>
                        <textarea name="condicoes_pagamento" id="textarea-pagamento" class="d-none"></textarea>
                    </div>

                    <div class="mb-3">
                        <label class="form-label">Prazo de Entrega</label>
                        <input type="text" name="prazo_entrega" class="form-control" value="{{ old('prazo_entrega', $orcamento->prazo_entrega) }}">
                    </div>
                </div>
            </div>
            ...
            <div class="card mb-4 border-primary">
                ...
                <div class="card-body">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" step="0.01" min="0" name="desconto" id="input_desconto" class="form-control form-control-sm" value="{{ old('desconto', number_format($orcamento->desconto, 2, '.', '')) }}" oninput="calcularTotaisGlobais()">
                    </div>
                    <div class="mb-3">
                        <label class="form-label text-primary">Acréscimos / Frete (R$)</label>
                        <input type="number" step="0.01" min="0" name="frete_acrescimos" id="input_frete" class="form-control form-control-sm" value="{{ old('frete_acrescimos', number_format($orcamento->frete_acrescimos, 2, '.', '')) }}" oninput="calcularTotaisGlobais()">
                    </div>
                    <hr>
...
<script>
    ...
    $(document).ready(function() {
        if ($('.select2-bootstrap-5').length) {
            $('.select2-bootstrap-5').select2({ theme: 'bootstrap-5' });
        }
        ...
        // INJEÇÃO DOS ITENS SALVOS
        const itensSalvos = @json($orcamento->itens);
        if (itensSalvos && itensSalvos.length > 0) {
            itensSalvos.forEach(item => {
                adicionarLinhaItem(item.descricao, item.detalhes || '', parseFloat(item.quantidade), parseFloat(item.valor_unitario));
            });
        } else {
            adicionarLinhaItem();
        }
