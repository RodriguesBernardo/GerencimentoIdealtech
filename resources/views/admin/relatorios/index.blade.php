    ...
    <div class="container-fluid">
                        ...
                        <form action="{{ route('admin.relatorios.index') }}" method="GET" id="formFiltro">
                            <div class="d-flex flex-column flex-lg-row align-items-lg-end gap-3 justify-content-between">
                                <p class="text-muted mb-0 small">
                                    Exibindo dados de <span class="fw-bold ">{{ $inicioFormatado }}</span> até <span
                                        class="fw-bold ">{{ $fimFormatado }}</span>
                                </p>

                                <div class="btn-group shadow-sm" role="group">
                                    <button type="button" class="btn btn-outline-secondary"
                                        onclick="setPeriodo('mes_atual')">Este Mês</button>
                                    <button type="button" class="btn btn-outline-secondary"
                                        onclick="setPeriodo('mes_anterior')">Mês Passado</button>
                                    <button type="button" class="btn btn-outline-secondary"
                                        onclick="setPeriodo('ano_passado')">Ano Passado</button>
                                    <button type="button" class="btn btn-outline-secondary"
                                        onclick="setPeriodo('ano_atual')">Este Ano</button>
                                </div>

                                <div class="d-flex gap-2 align-items-end">
                                    <div>
                                        <label class="form-label text-muted small fw-bold mb-1">Início</label>
                                        <input type="date" class="form-control form-control-sm" name="data_inicial"
                                            id="data_inicial"
                                            value="{{ $dataInicio ?? now()->startOfMonth()->format('Y-m-d') }}">
                                    </div>
                                    <div>
                                        <label class="form-label text-muted small fw-bold mb-1">Fim</label>
                                        <input type="date" class="form-control form-control-sm" name="data_final"
                                            id="data_final" value="{{ $dataFim ?? now()->endOfMonth()->format('Y-m-d') }}">
                                    </div>
            ...
            <div class="col-xl-3 col-md-6">
                <div class="card border-0 shadow-sm rounded-4 h-100 overflow-hidden">
                    <div class="card-body p-4 position-relative">
                        ...
                        <h2 class="fw-bold mb-1">R$
                            {{ number_format($dadosRelatorios['resumo']['valor_total_arrecadado'], 0, ',', '.') }}</h2>
            ...
            <div class="col-xl-3 col-md-6">
                <div class="card border-0 shadow-sm rounded-4 h-100">
                    <div class="card-body p-4">
                        ...
                        <h2 class="fw-bold mb-1">R$
                            {{ number_format($dadosRelatorios['resumo']['ticket_medio'], 2, ',', '.') }}</h2>
            ...
            <div class="col-xl-3 col-md-6">
                <div class="card border-0 shadow-sm rounded-4 h-100">
                    <div class="card-body p-4">
                        ...
                        <h2 class="fw-bold mb-1">{{ number_format($dadosRelatorios['insights']['taxa_inadimplencia'], 1) }}%
                        </h2>
                                ...
                                <div class="d-flex justify-content-between align-items-center mb-2 p-2 rounded-3 hover-">
                                    <div class="d-flex align-items-center">
                                        <span class="badge rounded-circle p-1 me-2"
                                            style="background-color: {{ $statusColors[strtolower(str_replace(' ', '_', $label))] ?? '#6B7280' }}"></span>
                                        <span class="small fw-bold ">{{ $label }}</span>
                                    </div>
                                    <span class="small fw-bold">R$
                                        {{ number_format($dadosRelatorios['graficos']['status_pagamento']['valores'][$index], 2, ',', '.') }}</span>
                                </div>
                            @endforeach
                        </div>
                    </div>
        ...
        <div class="row g-4 mb-4">
            <div class="col-lg-4">
                <div class="card border-0 shadow-sm rounded-4 h-100">
                    <div class="card-header bg-transparent border-0 pt-4 px-4 pb-0">
                        <h5 class="fw-bold mb-0">Conversão de Vendas</h5>
                        <p class="text-muted small mb-0">Orçamentos que viraram pedidos</p>
                    </div>
                    <div class="card-body p-4 text-center">
                        <div style="height: 200px; position: relative;">
                            <canvas id="graficoConversaoOrcamentos"></canvas>
                            <div style="position: absolute; top: 55%; left: 50%; transform: translate(-50%, -50%);">
                                <h2 class="fw-bold mb-0">{{ $dadosRelatorios['orcamentos']['taxa_conversao'] }}%</h2>
                                <small class="text-muted fw-bold">TAXA</small>
                            </div>
                        </div>
                        <div class="mt-3">
                            <span class="badge bg-soft-success text-success px-3">
                                {{ $dadosRelatorios['orcamentos']['aprovados'] }} Aprovados
                            </span>
                            <span class="badge bg-soft-secondary text-secondary px-3">
                                {{ $dadosRelatorios['orcamentos']['total_orcamentos'] }} Total
                            </span>
                        </div>
                    </div>
                </div>
            ...
            <div class="col-lg-8">
                <div class="card border-0 shadow-sm rounded-4 h-100">
                    <div class="card-header bg-transparent border-0 pt-4 px-4 pb-0">
                        <h5 class="fw-bold mb-0">Status dos Orçamentos</h5>
                        <p class="text-muted small mb-0">Volume por situação atual</p>
                    </div>
                    <div class="card-body p-4">
                        <div style="height: 250px;">
                            <canvas id="graficoStatusOrcamentosBarras"></canvas>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="card border-0 shadow-sm rounded-4 bg-primary text-white"
                    ...
                    <div class="card-body p-4">
                        ...
                        <div class="row g-4">
                            <div class="col-md-3">
                                <div
                                    class="p-3 bg-white bg-opacity-10 rounded-3 h-100 border border-white border-opacity-25">
                                    <small class="text-white-50 text-uppercase fw-bold ls-1">Melhor Cliente</small>
                                    <h5 class="mt-2 mb-1 fw-bold">
                                        {{ $dadosRelatorios['insights']['melhor_cliente']->nome ?? 'N/A' }}</h5>
                                    <small class="text-white">
                                        @if (isset($dadosRelatorios['insights']['melhor_cliente']))
                                            Gerou R$
                                            {{ number_format($dadosRelatorios['insights']['melhor_cliente']->servicos_sum_valor, 2, ',', '.') }}
                                        @endif
                                    </small>
                                </div>
                            </div>
                            <div class="col-md-3">
                                <div
                                    ...
                                    <h5 class="mt-2 mb-1 fw-bold">
                                        {{ Str::limit($dadosRelatorios['insights']['servico_mais_lucrativo']->descricao ?? 'N/A', 20) }}
                                    </h5>
                                    <small class="text-white">
                                        @if (isset($dadosRelatorios['insights']['servico_mais_lucrativo']))
                                            Total: R$
                                            {{ number_format($dadosRelatorios['insights']['servico_mais_lucrativo']->total, 2, ',', '.') }}
                                        @endif
                                    </small>
                                </div>
                            </div>
                            <div class="col-md-3">
                                <div
                                    ...
                                    <h5 class="mt-2 mb-1 fw-bold">
                                        {{ $dadosRelatorios['insights']['dia_semana_mais_produtivo'] }}</h5>
                            ...
                            <div class="col-md-3">
                                <div
                                    ...
                                    <h5 class="mt-2 mb-1 fw-bold">R$
                                        {{ number_format($dadosRelatorios['insights']['previsao_faturamento'], 0, ',', '.') }}
                                    </h5>
                                        ...
                                        <tr class="border-bottom border-light">
                                            <td class="px-4 py-3">
                                                <div class="d-flex align-items-center">
                                                    ...
                                                    <span
                                                        class="fw-bold ">{{ \Carbon\Carbon::parse($servico['data'])->format('d/m/Y') }}</span>
                                                </div>
                                            </td>
                                            <td class="py-3">
                                                <span
                                                    class="d-block fw-bold ">{{ \Illuminate\Support\Str::limit($servico['cliente'], 25) }}</span>
                                            </td>
                                            <td class="py-3">
                                                <span class="fw-bold text-primary">R$
                                                    {{ number_format($servico['valor'], 2, ',', '.') }}</span>
                                            </td>
                                            <td class="py-3">
                                                @php
                                                ...
                                                @endphp
                                                <span
                                                    class="badge bg-soft-{{ $statusClass }} text-{{ $statusClass }} rounded-pill px-3 py-2">
...
@push('scripts')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            new Chart(document.getElementById('graficoStatusPagamento'), {
                ...
                data: {
                    ...
                    datasets: [{
                        ...
                        backgroundColor: [colors.success, colors.warning, colors.danger, colors
                            .info
                        ],
                        borderWidth: 0,
                        hoverOffset: 10
            ...
            // 5. Gráfico de Conversão (Donut Simples)
            new Chart(document.getElementById('graficoConversaoOrcamentos'), {
                type: 'doughnut',
                data: {
                    labels: ['Aprovados', 'Não Convertidos'],
                    datasets: [{
                        data: [
                            {{ $dadosRelatorios['orcamentos']['aprovados'] }},
                            {{ $dadosRelatorios['orcamentos']['outros'] }}
                        ],
            ...
            // 6. Gráfico de Barras Status de Orçamentos
            new Chart(document.getElementById('graficoStatusOrcamentosBarras'), {
                type: 'bar',
                data: {
                    labels: @json($dadosRelatorios['orcamentos']['status_labels']),
                    datasets: [{
                        label: 'Quantidade',
                        data: @json($dadosRelatorios['orcamentos']['status_quantidades']),
                        backgroundColor: (context) => {
                            const label = context.chart.data.labels[context.dataIndex];
                            if (label === 'Aprovado') return colors.success;
                            if (label === 'Vencido' || label === 'Rejeitado') return colors
                                .danger;
